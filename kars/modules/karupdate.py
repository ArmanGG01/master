...
async def deploy(xx, repo, ups_rem, ac_br, txt):
    ...
    await edit_or_reply(
        xx,
        "**Seperti Nya, KAR-UBOT Mu Sudah Selesai Di Apdet, Silahkan Tunggu beberapa Saat Sampai Ada notif Dari KAR-UBOT, Setelah itu Test kars mu, jika tidak Bekerja Bisa Bertanya Ke @obrolansuar**",
    )
...
async def upstream(event):
    ...
    try:
        txt = (
            "**Perapdetan gagal di lanjutkan ajg.. "
            + "Ada beberapa kendala ni tot**\n\n**LOGTRACE:**\n"
        )
        repo = Repo()
