...
#
# Thanks for @mrismanaziz @Man-Userbot
import asyncio
import io
import os
...
async def cevir(event):
    ...
    try:
        ...
        return
    if rambot in ["foto", "photo"]:
        rep_msg = await event.get_reply_message()
        if not event.is_reply or not rep_msg.sticker:
            await edit_delete(event, "**Harap balas ke stiker.**", 10)
            return
        pornhub = await edit_or_reply(event, "`Mengconvert ke foto...`")
        foto = io.BytesIO()
        ...
        await pornhub.delete()
        os.remove("sticker.png")
    elif rambot in ["sound", "audio"]:
