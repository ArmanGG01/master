...
from userbot import CMD_HELP, BLACKLIST_CHAT, ramuser, CMD_HANDLER as cmd
...
async def roas(roaspl):
    if roaspl.chat_id in BLACKLIST_CHAT:
        return await edit_or_reply(
            roaspl, f"**Dasar {ramuser} ngentot, Lo gabisa Nyoba disini anjing!!!**"
        )
...
async def _(event):
    if event.chat_id in BLACKLIST_CHAT:
        return await edit_or_reply(
            event, f"**Dasar {ramuser} ngentot, Lo gabisa Nyoba disini anjing!!!**"
        )
...
async def _(event):
    ...
    await event.client.send_message(
        event.chat_id,
        "**UDAH LAH, MENDING LO CABUT AJA DARI SINI, SOAL NYA GUA GAMAU ANJING LIAT MANUSIA SAMPAH KAYAK LO ADA DI TELEGRAM, LEBIH TEPAT NYA MENDING LO DOWNLOAD MICHAT NAH ABIS TU LU NGELOTE ANJING, COCOK BANGET SOAL NYA BUAT LO ASU!!!!**")
    await event.delete()
...
async def _(event):
    ...
    await event.client.send_message(
        event.chat_id,
        "**GAUSAH SOK SOK AN NYENGGOL GUA ANJING, TAR GUA AJAKIN SESI, LO LARI, GUA AJAKIN BIKIN KONSEKUENSI LO BANYAK ALIBI, YAHAHA SAMPAH BANGET LO NGENTOD, UDAH LAH MENDING LO PERGI JAUH JAUH DARI MATA GUA, JIJI BAT LIATNYA CUIHHKKKK.**")
    await event.delete()
